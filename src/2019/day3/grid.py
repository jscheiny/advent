def moveInDirection(point, direction):
    (x, y) = point
    if direction == "U":
        return (x, y+1)
    if direction == "D":
...
def getStepPoints(origin, step):
    (direction, distance) = step
    points = []
    currentPoint = origin;
    for i in range(distance):
        currentPoint = moveInDirection(currentPoint, direction)
        points.append(currentPoint);
    return points;

def getPointsOnPath(steps):
    points = [(0, 0)]
    currentPoint = points[0]
    for step in steps:
        points.extend(getStepPoints(currentPoint, step))
...
def parsePath(str):
    return list(map(parseStep, str.split(",")))
...
def neighbors(point, allPoints):
    (x,y) = point
    potentialNeighbors = set([(x+1,y), (x-1,y), (x,y+1), (x,y-1)])
    return allPoints & potentialNeighbors

def shortestPath(allPoints, source, destination):
    dist = dict.fromkeys(allPoints, float("inf"))
    prev = dict.fromkeys(allPoints, None)
    unvisited = set(allPoints)

    dist[source] = 0

    while len(unvisited) != 0:
        u = argmin(unvisited, lambda pt: dist[pt])
        unvisited.remove(u)

        for v in neighbors(u, allPoints):
            alt = dist[u] + 1
            if alt < dist[v]:
                dist[v] = alt
                prev[v] = u
...
def solve(a, b):
    aPoints = getPointsOnPath(parsePath(a))
    bPoints = getPointsOnPath(parsePath(b))
    commonPoints = set(aPoints) & set(bPoints)
    source = (0, 0)
    commonPoints.remove(source)
    dists = []
    for destination in commonPoints:
        aDist = stepsToPoint(aPoints, destination)
        bDist = stepsToPoint(bPoints, destination)
        dists.append(aDist + bDist)  
    print(min(dists))  
